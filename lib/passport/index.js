...
Passport.prototype.getStrategySettings = function(appId, provider,callback){
  ...
  switch(provider){
    case 'facebook':
      schema = StrategySchema.FacebookStrategySettings;
      break;
    case 'google':
      schema = StrategySchema.GoogleStrategySettings;
      break;
    case 'twitter':
      schema = StrategySchema.TwitterStrategySettings;
      break;
    default:
      break;
  }

  return StrategySchema.findSettings(schema, appId, callback);
}
...
var UserSchema = new Schema({username:String, password:String},{strict:false});
...
Passport.prototype.initStrategy = function(req, res, next) {
  ...
  this.getStrategySettings(req.params.appId, req.params.provider, function(result) {
    ...
    _this.strategyFactory(req.params.appId, req.params.provider, result);    

    _this.authenticate(req.params.appId+'-'+req.params.provider, {scope : 'https://www.google.com/m8/feeds https://www.googleapis.com/auth/userinfo.email https://www.googleapis.com/auth/userinfo.profile'})(_req, _res, _next);
...
Passport.prototype.strategyFactory = function(appId, providerName, result){
          ...
          case 'twitter':
                  strategy = new TwitterStrategy({                  
                                ...
                                function(token, tokenSecret, profile, done) {
                                  ...
                                  User.findOne( {displayName : profile.displayName }, function(err, user) {
                                    if (err) { return done(err); }
                                    if (!user){
                                      user = new User(profile);
                                      user.save();                                      

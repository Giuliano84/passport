...
var fs = require('fs')
  ...
  , StrategySchema = require('./middleware/strategySchema')
  , userSchema = require('./middleware/userSchema')
  , User = require('./middleware/userSchema').User;
...
Passport.prototype.getStrategySettings = function(appId, provider,callback){

  return StrategySchema.findSettings(appId, provider, callback);
}
...
/**
 * Passport Strategies extensions.
 */
var FacebookStrategy = require('passport-facebook').Strategy;
var GoogleOpenIDStrategy = require('passport-google').Strategy;
var GoogleStrategy = require('passport-google-oauth').OAuth2Strategy;
var TwitterStrategy = require('passport-twitter').Strategy;


Passport.prototype.initStrategy = function(req, res, next) {
  var _next = next;
  var _this = this;
  var _req = req;
  var _res = res;

  this.getStrategySettings(req.params.appId, req.params.provider, function(result) {

    _this.strategyFactory(req.params.appId, req.params.provider, result, _this.loginCallback);

    _this.authenticate(req.params.appId+'-'+req.params.provider, {scope : result.params.scope} )(_req, _res, _next);

  })
}


Passport.prototype.execStrategy = function(req, res, next) {
  var _next = next;
  var _req = req;
  var _res = res;

  this.authenticate( req.params.appId+'-'+req.params.provider,
                     {
...
Passport.prototype.loginCallback = function(accessToken, refreshToken, profile, done, appId, providerName){
  console.log('Logged with '+providerName+':');
  console.log(profile);
  var key = "identities."+providerName+"."+providerName+"Uid";

  var queryObject = {};
  queryObject[key] = profile._json.id;
  User.findOne(queryObject, function (err, user) {
    if (err) { return done(err); }
    if (!user){
      console.log('utente non presente')
      user = userSchema.createUser(appId, providerName, profile, accessToken);
      user.save();
    }
...
Passport.prototype.strategyFactory = function(appId, providerName, result, loginCallback){
  var strategy;
  ...
  switch (providerName) {
        case 'facebook':
                  strategy = new FacebookStrategy({
                                ...
                                clientSecret: result.params.clientSecret,
                                callbackURL: result.params.callbackUrl,
                              },
                              function(accessToken, refreshToken, profile, done) {
                                return loginCallback(accessToken, refreshToken, profile, done, appId, providerName);
                              });
                  break;
          ...
          case 'twitter':
                  strategy = new TwitterStrategy({
                                  consumerKey: result.params.consumerKey,
                                  consumerSecret: result.params.consumerSecret,
                                  callbackURL: result.params.callbackUrl
                                },
  ...
  this.use(appId+'-'+providerName, strategy);
}
